...
def merge_Receipt(date, address_2, seller,
    ...
    document.merge(date=date, address_2=address_2, seller=seller,
    ...
    for i in range(0, len(itemList) - 2, 3):
        ...
        tot+=int(itemList[i + 2]) * int(itemList[i + 1])
    document.merge_rows('item', sales_history)
    test = document.merge(total_total=tot)
    convert(test, "static/PDF/Receipt.pdf")
